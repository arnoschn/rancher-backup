...
class Backup(object):
    ...
    def dumpRancherDatabase(self, backupPath, listDatabaseSettings):
        ...
        command = "sh -c 'mysqldump --single-transaction --max_allowed_packet=512M --quick -h %s -P %s -u %s %s > %s/%s.dump'" % (listDatabaseSettings['host'], listDatabaseSettings['port'], listDatabaseSettings['user'], listDatabaseSettings['name'], target_dir, listDatabaseSettings['name'])
